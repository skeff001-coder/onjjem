...
import { AlertCircle, ArrowRight, Dog, Heart } from "lucide-react";
...
export default function Pets() {
  ...
  const petKeywords = ["dog", "pet", "canvas", "mug", "framed", "jigsaw", "coaster", "puzzle", "cushion", "eco"];

  const filterProducts = () => {
    if (!products) return [];
    const petProducts = products.filter((p: any) =>
      petKeywords.some((k) => p.name.toLowerCase().includes(k))
    );
    ...
    if (productHint) {
      ...
      const matched = petProducts.filter((p: any) =>
        hints.some((h) => p.name.toLowerCase().includes(h.toLowerCase()))
      );
      return matched.length ? [...matched, ...petProducts.filter((p: any) => !matched.includes(p))] : petProducts;
    }
    ...
    <div className="container mx-auto px-4 py-12 md:py-24 max-w-6xl">
      ...
      <div className="max-w-2xl mb-16">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-sm font-semibold text-primary uppercase tracking-wider">Pets Collection</span>
        </div>
        <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-6 text-foreground">
          For dogs, <span className="text-primary">by dog lovers</span>.
        </h1>
        <p className="text-lg md:text-xl text-muted-foreground leading-relaxed">
          Every item here is designed for your dog. Upload a photo and we will print it on premium merch made just for pets.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center rounded-2xl border border-destructive/20 bg-destructive/5">
          ...
          <p className="text-muted-foreground mb-6">There was a problem fetching the pet catalog.</p>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center rounded-2xl border border-border/50 bg-muted/20">
          ...
          <h3 className="text-lg font-semibold mb-2">Pet products coming soon</h3>
          <p className="text-muted-foreground">Check back soon for dog-themed merch.</p>
        </div>
      ) : (
