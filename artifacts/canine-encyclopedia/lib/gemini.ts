...
export async function identifyBreedFromBase64(
  ...
  if (!response.ok) {
    ...
    throw new Error((err as any)?.error ?? `Breed scan failed: ${response.status}`);
  }
...
export async function getGlowup(breed: string, style: string, dogName?: string): Promise<GlowupResult> {
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error((err as any)?.error ?? `Glow-Up failed: ${response.status}`);
  }
...
export async function getBreedKnowledge(breed: string): Promise<BreedKnowledge> {
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error((err as any)?.error ?? `Knowledge fetch failed: ${response.status}`);
  }
...
export async function getMixedBreedDNA(
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error((err as any)?.error ?? `Mixed breed DNA failed: ${response.status}`);
  }
  return response.json();
}
...
export async function getAgeEstimate(
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error((err as any)?.error ?? `Age estimate failed: ${response.status}`);
  }
  return response.json();
}
...
export async function getPersonalityScan(
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error((err as any)?.error ?? `Personality scan failed: ${response.status}`);
  }
  return response.json();
}
...
export async function getHealthGuide(breed: string, dogName?: string): Promise<HealthGuideResult> {
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error((err as any)?.error ?? `Health guide failed: ${response.status}`);
  }
  return response.json();
}
...
export async function getTrickTrainer(breed: string, dogName?: string): Promise<TrickTrainerResult> {
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error((err as any)?.error ?? `Trick trainer failed: ${response.status}`);
  }
  return response.json();
}
