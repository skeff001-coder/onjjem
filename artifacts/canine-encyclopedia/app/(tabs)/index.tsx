  ...
  return useMemo(
    () => [
      ...
      },
      {
        ...
        subtitle: "Personalised wellness plan",
        description: "Get tailored health tips, exercise plans, and diet advice. Includes ONJJEM product recommendations for your breed.",
        icon: "medical-outline",
        color: "#4ade80",
        glow: "rgba(74,222,128,0.35)",
        free: false,
        packageId: PACKAGE_HEALTH_GUIDE,
        entitlementCheck: () => hasHealthGuide || all,
      },
      {
        id: "trick_trainer",
        title: "Trick Trainer",
        subtitle: "Fun personalised training",
        description: "A custom trick training plan tailored to your breed's intelligence and ability. Build a stronger bond.",
        icon: "flash-outline",
        color: "#FFE600",
        glow: "rgba(255,230,0,0.35)",
        free: false,
        packageId: PACKAGE_TRICK_TRAINER,
        entitlementCheck: () => hasTrickTrainer || all,
      },
    ],
  ...
  const processImage = async (uri: string, base64: string, scanType: ScanType) => {
    ...
    try {
      switch (scanType) {
        ...
        case "mixed_dna": {
          const data = await getMixedBreedDNA(base64, "image/jpeg");
        ...
        case "age_calc": {
          const data = await getAgeEstimate(base64, "image/jpeg");
        ...
        case "personality": {
          const data = await getPersonalityScan(base64, "image/jpeg");
